...
class DayNightAnim() {
    ...
    fun smoothDayNightFadeOut(
        bind : ActivityMainBinding,
        bindPlayer : StubPlayerActivityMainBinding?
    ){
        ...
        if(MainActivity.uiMode != Configuration.UI_MODE_NIGHT_YES){
            ...
            val colorFrom = ContextCompat.getColor(context, R.color.nav_bar_settings_frag)
    ...
    fun smoothDayNightFadeIn(bind : ActivityMainBinding,
                             bindPlayer : StubPlayerActivityMainBinding?){
        val context = bind.root.context
